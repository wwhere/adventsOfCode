...
class Link:
    def __init__(self, a: Node, b: Node, cost: int, facing: str, visited: list[tuple[int, int]], positions: list[tuple[int, int]]):
        self.a: Node = a
        self.b: Node = b
        self.cost = cost
        self.facing = facing
...
def find_links(grid, node, current_facing, facings, visited):
    ...
    for facing in facings:
        ...
        if is_wall(grid, first_pos):
            continue
        cost += 1
        possible, next_node, next_facing, node_cost, positions = find_node(grid, first_pos, facing)
        if possible and not next_node.pos() in visited:
            cost += node_cost
            new_visited = visited + [node.pos()]
            link = Link(node, next_node, cost, next_facing, new_visited, positions)
            links.append(link)

    return links


def find_node(grid, pos, facing) -> tuple[bool, Node, str, int, list[tuple[int, int]]]:
    cost = 0
    positions = [pos]
    current_pos = pos
    ...
    while len(next_steps) == 1:
        ...
        if is_end(grid, current_pos):
            ...
            return True, node, current_facing, cost, positions
        next_steps = get_next_steps(grid, current_pos, current_facing)

    if len(next_steps) == 0:
        return False, None, '', cost, positions
...
def get_solution(grid, facing, start, end):
    ...
    while len(links) > 0:
        ...
        link = links.pop()
        if link.b.pos() == end:
            if (best_cost_to_end != -1 and best_cost_to_end == link.cost) or (best_cost_to_end == -1):
                positions = positions + link.positions
                best_cost_to_end = link.cost
                continue
        ...
        for l in next_links:
            l.cost += link.cost
            l.positions += link.positions
            current_cost_to_node = paths_to_nodes.get(l.b.pos(), -1)
            if current_cost_to_node != -1 and current_cost_to_node + TURN_90_POINTS < l.cost or (best_cost_to_end != -1 and l.cost >= best_cost_to_end):
                continue
            links.append(l)
            paths_to_nodes[l.b.pos()] = l.cost

    return len(list(dict.fromkeys(positions)))+1
...
if __name__ == "__main__":
    assert process('d16.4.data') == 8
    assert process('d16.1.data') == 45
    assert process('d16.2.data') == 64
    process('d16.3.data')
